...
uint16_t tempCalc(cell_asic *bms_ic, uint8_t ic, uint8_t temp)
{
    ...
    double kV_Ref2; // Reference voltage for temperature conversions

    kV_Ref2 = bms_ic[ic].aux.a_codes[5] * 0.0001;
    R = bms_ic[ic].aux.a_codes[temp] / (kV_Ref2 - (bms_ic[ic].aux.a_codes[temp] * 0.0001));
...
float readCurrent()
{
    const float kSensorSensitivity = 0.0057; // Sensor sensitivity in V/A
    const float kV_Ref = 2.4929;              // Reference voltage
    const float kAnalogResolution = 5.0 / 1023.0;

    uint16_t currentValue = analogRead(CSOUT);            // changed from int to uint16_t
    float voltage = currentValue * kAnalogResolution;      // Converts to Voltage
    float current = (voltage - kV_Ref) / kSensorSensitivity; // Calculates Current
    return current;
}