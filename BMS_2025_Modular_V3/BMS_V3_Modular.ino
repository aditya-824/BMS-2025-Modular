...
#define GUI 1    // 0: Serial output, 1: GUI output
#define DEBUG 0  // 1: Debug data shown, 0: No debug data
/* ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
...
void loop() {
  ...
  if (millis() - next_loopTime >= MEASUREMENT_LOOP_TIME) {
    ...
    if (mainLoopCounter >= 10) {
      ...
      bool tempFault = temperature_faultCheck(TOTAL_IC, TEMPS, temps_faultCounter, tempfaultStatus, BMS_IC, GUI);
      bool csFault = csFault_check(GUI);
      ...
      AvgBMSData();
      minmaxCV(BMS_IC, TOTAL_IC, TOTAL_CELL, GUI);
      minmaxCT(BMS_IC, TOTAL_IC, TEMPS, GUI);
      ...
      if (canSend(BMS_IC, TOTAL_IC, TOTAL_CELL, TEMPS, allData, vsHVin, vsBat, soc, currentReading) == true) {
        if (GUI) {
          Serial.print("CANDataSent, ");
        }
        Serial.println("DATA SENT SUCCESSFULLY");
      } else {
        if (GUI) {
          Serial.print("CANDataNotSent, ");
        }
      }
      if (chargerCAN_status == true) {
        if (GUI) {
          Serial.print("ChargerCANIDRecieved, ");
        } else {
          Serial.println("Recieved Correct CAN ID from charger.");
        }
      } else {
        if (GUI) {
          Serial.print("ChargerCANIDNotRecieved, ");
        } else {
...
void AvgBMSData() {
  for (uint8_t currentIc = 0; currentIc < TOTAL_IC; currentIc++) {
    ...
    for (uint8_t cell = 0; cell < TOTAL_CELL; cell++) {
      ...
      print(currentIc, cell, avg_cellVoltage, isTemp, voltfaultStatus[currentIc][cell], GUI);
    ...
    for (uint8_t temp = 0; temp < TEMPS; temp++) {
      ...
      print(currentIc, temp, avg_cellTemperature, isTemp, tempfaultStatus[currentIc][temp], GUI);
  ...
  printStackVoltage(TOTAL_IC, BMS_IC, GUI);
  if (!GUI) {
    Serial.println("----------------------------------------------------------------------------------------------------------------------------------------------------------");
  }
}
