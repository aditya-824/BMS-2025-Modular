...
void printStackVoltage(uint8_t total_ic, cell_asic *BMS_IC, bool gui)
{
    ...
    if (gui)
    {
        ...
        {
            float voltage = BMS_IC[current_ic].stat.stat_codes[0] * 0.0001 * 20;
            packVoltage += voltage;
...
void print(uint8_t ic, uint8_t cell, uint16_t value, bool temp = false, bool fault = false, bool gui)
{
    if (gui)
    {
        if (temp == false)
